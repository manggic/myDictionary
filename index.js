...
function myFunction() {
  ...
  for(let i=97; i<=122; i++){
    ...
    document.getElementById('myContainer').innerHTML +=  `<div id=${i-96} class="buttons"><button class="button grow" data-toggle='modal' data-target='#exampleModal${i-96}' >${code}</button>
     <button id=${code}${code}    data-toggle='modal' data-target='#exampleModal${code}${code}' class="miniButton">+</button></div>`
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content bg-primary">
            ...
            <div class="modal-body">
               <table id=${ string } class="table table-dark">
                 <thead>
                   <tr>
                       <th>English</th>
                       <th>Meaning</th>
      ...
      for(let i=1;i<=26;i++){
        ...
        `<div class="modal fade" id="exampleModal${string}${string}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content elementModal bg-info">
              ...
              <div class="modal-body">
               <h5> how many element u want to add ??</h5>
               <input  type="number" class='input'  oninput="changeinput('${string}')"  id="input${string}" placeholder="Type Number" >
              </div>
            ...
            // localStorageData.push(dataInObject)   
              console.log(dataInObject, 'length', dataInObject.length)  
               ...
               for(let i=0;i< len ;i++){
                ...
                document.getElementById(string).innerHTML +=  `<tbody><tr><td>${ dict['key'] }</td> 
                                 <td>${ dict['value'] }</td>
                                 <td><button onclick="deleteOne(id, name)"  name=${dict['key']}   id=${string} class="btn btn-danger btn-sm">X</button></td></tr></boody><br><br>`
              }
            }
           else{
      ...
      // alert(document.getElementById(`input${letter}`).value)
       ...
       fullinput = ``
        for(let i=1;i <= element;i++){
               fullinput +=
               `<div class='single'><div class="row" >
...
function OnSubmit(letter){
      ...
      for(let i=0;i< len ;i++){
        ...
        if( meaningValue && englishValue){
          document.getElementById(letter).innerHTML +=  `<tbody><tr><td>${(English[i].value)}</td> 
          <td>${(meaning[i].value)}</td>
          <td><button onclick="deleteOne(id, name)"  name=${(English[i].value)}   id=${letter} class="btn btn-danger btn-sm">X</button></td></tr></boody><br><br>`
      ...
      if( localStorageData[letter]!== undefined )
         localStorage.setItem(letter,  JSON.stringify(localStorageData[letter].concat(localStorageDat)) )
